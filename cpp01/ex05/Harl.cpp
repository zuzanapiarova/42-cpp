...
{
    // this is how pointers to member functions are created and called
    // void (Harl::*testFunction)() = &Harl::debug;
    // (this->*testFunction)();

    std::string levels[] = {"DEBUG", "INFO", "WARNING", "ERROR"};
    void (Harl::*functionPointers[])() = {&Harl::debug, &Harl::info, &Harl::warning, &Harl::error};

    for (int i = 0; i < 4; ++i)
    {
        if (level == levels[i])
        {
            (this->*functionPointers[i])(); // here function is called from within the class
            return ;
        }
    }
